...
# Experment 1 
python /home/nkd/ouyangzl/conditional-flow-matching/runner/src/train.py \
  --multirun \
  experiment=cfm \
  model=[cfm,otcfm] \
  model.sigma_min=0.1 \
...
# Experiment 2
python /home/nkd/ouyangzl/conditional-flow-matching/runner/src/train.py --multirun experiment=cfm \
  model=[fm] \
  model.sigma_min=0.1 \
  datamodule=[scurve,moons,gaussians] \
  seed=42,43,44,45,46 &