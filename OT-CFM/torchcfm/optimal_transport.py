...
class OTPlanSampler:
    ...
    def get_map(self, x0, x1):
        ...
        """
        a, b = pot.unif(x0.shape[0]), pot.unif(x1.shape[0])  # 1 / n
        if x0.dim() > 2:   # Reshape to 2-dim tensor
            x0 = x0.reshape(x0.shape[0], -1)
        if x1.dim() > 2:
            x1 = x1.reshape(x1.shape[0], -1)
        M = torch.cdist(x0, x1) ** 2   # [8, 8]   # Euclidean cost
    ...
    def sample_plan(self, x0, x1, replace=True):
        ...
        x1[j] : Tensor, shape (bs, *dim)
            represents the source minibatch drawn from $\pi$
        """
        pi = self.get_map(x0, x1)  # [8, 8]  # bijective mapping 
        i, j = self.sample_map(pi, x0.shape[0], replace=replace)  # one to one correspondence
        return x0[i], x1[j]
