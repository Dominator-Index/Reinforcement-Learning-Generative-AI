...
@hydra.main(config_path="/home/nkd/ouyangzl/Reinforcement-Learning-Generative-AI/CleanDiffuser/configs_reconstruct/conditionalflowmatching/kitchen", config_name="kitchen", version_base=None)
def pipeline(args):
    # ------------------ 初始化和配置 ------------------
    set_seed(args.seed)
    ...
    if args.mode == "train":
        ...
        sample_bs, preserve_bs, ptr = 10, 10, 0
            ...
            for i in range(args.num_episodes):
                ...
                while not np.all(cum_done) and t < (280 + 1):
                    ...
                    traj, log = agent.flow_matching_sample(
                        ...
                        n_samples=args.num_candidates,  # 64 
                    ...
                    act = act_tensor.clip(-1., 1.).cpu().detach().numpy()
